...
class NetworkUndefinedError extends Error {
    ...
    constructor(chainId: number, name?: string) {
        super(`Connected Ethereum network is not supported: ${name ?? ''} (${chainId})`)
 ...
 */
export const useEthereumNetworkOptions = (chainId?: number): UseEthereumNetworkOptionsResult => {
    ...
    return useMemo<UseEthereumNetworkOptionsResult>(() => {
        ...
        if (result !== undefined) {
            ...
            return {
                error: new NetworkUndefinedError(chainId, chainId === network?.chainId ? network.name : undefined),
            }
        }
